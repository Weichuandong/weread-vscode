...
[![Release](https://img.shields.io/github/v/release/Weichuandong/weread-vscode?color=blue)](https://github.com/Weichuandong/weread-vscode/releases)
[![Open VSX](https://img.shields.io/open-vsx/v/weichuandong/weread-vscode?label=open%20vsx&color=purple)](https://open-vsx.org/extension/weichuandong/weread-vscode)
...
在 VSCode 侧边栏阅读微信读书：书城 / 书架 / 章节目录 / 正文 / 想法划线书评 / 云端进度多端同步。

#### 书城（发现新书）

- 🔍 **全网搜书** —— 搜书名 / 作者，结果带封面、评分、在读人数、简介，支持"加载更多"翻页
- 🔃 **结果排序** —— 综合 / 评分 / 在读 / 热度 / 价格 一键切（客户端排序，微信读书服务端不支持排序参数）。「在读」看当下热度，「热度」看累计评分人数（老经典占优），评分同分时评分人数多的靠前
- 🏆 **榜单 & 分类（官方全量）** —— 7 个榜单（飙升 / 新书 / 小说榜 / 总榜 / 神作榜 / 神作潜力榜 / 热搜榜）+ **20 个一级分类 + 143 个二级分类**，三行 chips 逐级下钻：题材行默认露 8 个，点「更多 ▾」全铺；选中题材后自动展开它的子类（如「计算机 → 软件学习 / 编程设计 / 理论知识 …」）
- ➕ **一键加入书架** —— 卡片上直接加书架，加完立刻变「✓ 已在书架」
- 📖 **点开就读** —— 「开始阅读」直接进侧栏阅读器，复用章节缓存 / 云端进度全套能力
- 🆓 **免登录浏览** —— 搜索与榜单都不需要 Cookie，未登录也能逛；真正要读或加书架时才提示登录

#### 书架

- 顶部三 Tab 切换：「**书架**」/「**书城**」/「**在读**」
- 🔎 **本地搜索** —— 按书名 / 作者过滤（纯本地不打接口），搜索时分组自动展开，Esc 清空
- 🗃️ **分组方式可切换** —— 「云端分组」（微信读书 APP 里自建的 archive）/「阅读状态」（在读 / 已读完 / 未开始）/「不分组」
- ↕️ **排序** —— 默认 / 最近阅读 / 阅读进度 / 书名（按拼音），分组与排序偏好本地持久化
- 支持 **单组折叠/展开**、**全部展开**、**全部折叠**，分组折叠状态本地持久化
- 「在读」列表来自微信读书云端最近阅读

> 分组的**创建 / 重命名 / 移动书籍**请在微信读书 APP 或网页版里操作，插件这边刷新即可同步 —— 微信读书 web 端没有开放分组管理接口，插件也不会自造一套跟 APP 对不上的本地分类。
...
3. 书架加载后，点任意书进入阅读；想找新书就切「书城」tab（或命令面板 `Folio: 搜索书城`）搜书 / 逛榜单
4. 阅读页：
   - 顶部 ← / → 翻章节
...
| `weread.openStore` | Folio: 打开书城 (榜单 / 分类) |
| `weread.searchBooks` | Folio: 搜索书城 |
